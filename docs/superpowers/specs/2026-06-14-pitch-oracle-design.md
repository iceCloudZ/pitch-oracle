...
| `manual` | 人类经 CLI / 看板表单输入 | 用户本人，赛前输入；缺则记 DNF |
...
输入：`Prediction.resultProbs`（或 `scoreProbs`）+ 抓来的赔率。
...
```ts
interface Bet {
  market: 'home' | 'draw' | 'away' | string   // 可扩展大小球等
  modelProb: number
  impliedProb: number
  decimalOdds: number
  ev: number
...
附 **Brier 校准分**（基于 `resultProbs`）作为极客向副指标。

### 7.2 投注盈亏榜
每笔价值投注按**实际结果 × 实际赔率**结算，从虚拟本金扣除/返还，累计 **P/L（单位）**。

两榜均按 agent 汇总，三类 agent 同榜。看板同时展示：总积分、命中率、Brier、累计投注 P/L、ROI。
...
- 单赛事（2026 世界杯）。
- 默认阵容：2–3 个 `llm` agent + 1 个 `manual`（用户本人）+ 1 个 `human-augmented`。
- 每日 `predict` + `score`。
- 双排行榜（准确度 + 投注盈亏）。
- 静态看板：排行榜 + 逐场预测 + 投注建议 + agent 日记。
- 核心数学（计分 / EV / 凯利）有测试覆盖。

**v2 留待**：多赛事并发、大小球/让球等更多市场、看板多语言、模型自校准。
